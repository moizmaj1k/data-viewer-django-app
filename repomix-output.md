...
Generated by Repomix on: 2025-11-09 22:36:10
...
`````markdown
This file is a merged representation of the entire codebase, combining all repository files into a single document.
Generated by Repomix on: 2025-11-09 21:27:10
...
{% block content %}
  ...
  <!-- Add Layer Modal -->
  <div id="layer-modal" class="layer-modal" aria-hidden="true" role="dialog" aria-modal="true" aria-labelledby="layer-modal-title" style="display:none">
    <div class="layer-modal__backdrop"></div>
    <div class="layer-modal__panel" role="document">
      <div class="layer-modal__head">
        <h3 id="layer-modal-title">Add custom layer</h3>
        <button id="layer-cancel" class="btn btn-outline" type="button" aria-label="Close">✕</button>
      </div>
      <div class="layer-modal__body">
        <p class="muted">Supported now: <b>KML</b>, <b>GeoJSON</b>. KMZ will show a helpful note unless JSZip is present.</p>
        <div class="field">
          <span>File</span>
          <input id="layer-file" type="file" accept=".kml,.kmz,.json,.geojson,application/vnd.google-earth.kml+xml,application/vnd.google-earth.kmz,application/geo+json,application/json">
        </div>
        <div class="field">
          <span>Layer name (optional)</span>
          <input id="layer-name" type="text" placeholder="My Layer">
        </div>
        <div class="layer-modal__actions">
          <button id="layer-add" class="btn btn-solid" type="button">Add layer</button>
        </div>
...
````markdown
This file is a merged representation of the entire codebase, combining all repository files into a single document.
Generated by Repomix on: 2025-11-06 22:44:16
...
{% block toolbar %}
  <div class="toolbar-compact">
    ...
    <!-- Add Layer icon button -->
    <button id="btn-add-layer" class="btn btn-outline" type="button" title="Add Layer" aria-haspopup="dialog" aria-controls="add-layer-modal">
      <!-- simple plus-in-a-map icon -->
...
{% block content %}
  ...
  <!-- Modal markup (kept here; styled & controlled by layers.css/layers.js) -->
  <!-- Keep these AFTER scripts so our JS can find them on first paint -->
  <div id="add-layer-backdrop" class="al-backdrop" hidden></div>
  <div id="add-layer-modal" class="al-modal" role="dialog" aria-modal="true" aria-labelledby="al-title" hidden>
    <div class="al-card">
      <header class="al-head">
        <h3 id="al-title">Add a Map Layer</h3>
        <button id="al-close" class="btn btn-outline" type="button" aria-label="Close">✕</button>
      </header>
      <div class="al-body">
        <p class="al-help">Upload a <strong>KMZ</strong>, <strong>KML</strong>, or <strong>GeoJSON</strong> file. We’ll add it as a temporary vector layer and zoom to it.</p>
        <form id="al-form">
          <label class="al-file">
            <input id="al-file" name="file" type="file"
                   accept=".kmz,.kml,.geojson,.json,.gpx,.zip"
                   required>
          </label>
          <div class="al-options">
            <label class="check">
              <input id="al-autofit" type="checkbox" checked>
              <span>Fit map to layer extent</span>
            </label>
          </div>
          <div class="al-actions">
            <button id="al-cancel" type="button" class="btn btn-outline">Cancel</button>
            <button id="al-add"    type="submit" class="btn btn-solid">Add Layer</button>
          </div>
        </form>
...
(function () {
  ...
  // >>> Expose a narrow public surface for other scripts (layers.js)
  // Read-only reference to map & a helper to add layers safely.
  window.AppMap = Object.freeze({
    map,
    ol
  });

  // NEW: announce readiness so late/early scripts can reliably wait
  try {
    window.dispatchEvent(new CustomEvent('appmap:ready'));
  } catch (e) {
    console.warn('[dashboard] failed to dispatch appmap:ready', e);
  }
...
```css
/* Ensure anything with [hidden] is actually hidden in all browsers/resets */
[hidden] {
  display: none !important;
}

/* Screen-reader only helper from your existing style language */
.sr-only {
...
/* Backdrop that dims & blurs the page behind the modal */
.al-backdrop {
  position: fixed;
  inset: 0;
  background: rgba(15, 23, 42, 0.35);   /* slate dim */
...
.al-card {
  ...
  overflow: hidden;
  animation: al-pop .24s cubic-bezier(.22,.61,.36,1);
}
@keyframes al-pop {
...
.al-head h3 { margin: 0; font-size: 16px; }

.al-body { padding: 14px; }
.al-help { margin: 0 0 12px; color: #374151; font-size: 14px; }

.al-file input[type="file"] {
...
.al-options { margin-top: 10px; }
.al-options .check { display: inline-flex; gap: 8px; align-items: center; }
.al-options input { accent-color: #1d4ed8; }
...
```javascript
// layers.js — modal + file parsers (KMZ/KML/GeoJSON)
(function () {
  // Grab DOM first and wire up UI regardless of map readiness
  const btn       = document.getElementById('btn-add-layer');
  const modal     = document.getElementById('add-layer-modal');
  const backdrop  = document.getElementById('add-layer-backdrop');
  const closeBtn  = document.getElementById('al-close');
  const cancelBtn = document.getElementById('al-cancel');
  const form      = document.getElementById('al-form');
  const fileInput = document.getElementById('al-file');
  const autoFit   = document.getElementById('al-autofit');
  const errBox    = document.getElementById('al-error');

  // Defensive: if the template isn't on this page, bail silently
  if (!btn || !modal || !backdrop) return;

  // Make sure these start closed even if [hidden] is ignored by some reset
  ...
  // --- Waiter: resolve when the map is ready (event + polling fallback)
  function whenMapReady({ timeout = 8000 } = {}) {
    return new Promise((resolve, reject) => {
      // fast-path
      if (window.AppMap?.map && window.AppMap?.ol) return resolve(window.AppMap);
      let settled = false;
      const done = (val, isErr) => {
        if (settled) return;
        settled = true;
        clearInterval(iv);
        window.removeEventListener('appmap:ready', onEvt);
        isErr ? reject(val) : resolve(val);
      };
      const onEvt = () => {
        if (window.AppMap?.map && window.AppMap?.ol) done(window.AppMap, false);
      };
      window.addEventListener('appmap:ready', onEvt, { once: true });
      const t0 = Date.now();
      const iv = setInterval(() => {
        if (window.AppMap?.map && window.AppMap?.ol) return done(window.AppMap, false);
        if (Date.now() - t0 > timeout) return done(new Error('timeout'), true);
      }, 50);
    });
  }
  ...
  const WEBM  = 'EPSG:3857';
  const userLayers = [];

  function styleForVector() {
    return function (feature) {
      const g = feature.getGeometry();
      // ol will be available when we actually use the style (after map is ready)
      if (g instanceof window.AppMap.ol.geom.Point || g instanceof window.AppMap.ol.geom.MultiPoint) {
        return new ol.style.Style({
          image: new ol.style.Circle({
            radius: 5,
      ...
      return new ol.style.Style({
        stroke: new ol.style.Stroke({ color: '#fb923c', width: 3 }),
        fill: new ol.style.Fill({ color: 'rgba(251,146,60,0.15)' })
      });
    };
  }

  function addFeaturesAsLayer(features, nameHint) {
    if (!features || !features.length) throw new Error('No features found in file.');
    const src = new window.AppMap.ol.source.Vector({ features });
    const layer = new window.AppMap.ol.layer.Vector({ source: src, style: styleForVector(), zIndex: 25 });
    layer.set('title', nameHint || 'User layer');
    window.AppMap.map.addLayer(layer);
  ...
  function fitToLayer(layer) {
    try {
      ...
      if (ext && isFinite(ext[0])) {
        window.AppMap.map.getView().fit(ext, { padding: [30,30,30,30], duration: 300 });
      }
    } catch {}
  }

  async function readGeoJSON(text) {
    const fmt = new window.AppMap.ol.format.GeoJSON();
    return fmt.readFeatures(text, { dataProjection: WGS84, featureProjection: WEBM });
  }
  async function readKML(text) {
    const fmt = new window.AppMap.ol.format.KML();
    return fmt.readFeatures(text, { dataProjection: WGS84, featureProjection: WEBM });
  }
  async function readKMZ(arrayBuffer) {
    if (!window.JSZip) throw new Error('KMZ support requires JSZip (not loaded).');
  ...
  if (form) {
    form.addEventListener('submit', async (e) => {
      ...
      // Wait for the map right here (robust to script order)
      try {
        await whenMapReady({ timeout: 12000 });
      } catch {
        showError('Map is not ready yet. Try again in a second (scripts are still loading).');
        return;
      }
      ...
      try {
        ...
        if (ext === 'kmz') {
          ...
          features = await readKMZ(buf);
        } else if (ext === 'kml') {
          const text = await file.text();
          features = await readKML(text);
        } else if (ext === 'geojson' || ext === 'json') {
          const text = await file.text();
          features = await readGeoJSON(text);
        } else {
          throw new Error('Unsupported file type. Try KMZ, KML, or GeoJSON.');
        }

        const layer = addFeaturesAsLayer(features, name);
        if (autoFit && autoFit.checked) fitToLayer(layer);
        console.info('[layers] Added layer:', name, 'features:', features.length);
        closeModal();
      } catch (err) {
        console.error('[layers] add failed:', err);
...
```css
.layer-modal { position: fixed; inset: 0; z-index: 1000; }
.layer-modal[aria-hidden="true"] { display: none !important; }
...
.layer-modal__panel {
  ...
  border: 1px solid #e5e7eb; box-shadow: 0 20px 50px rgba(0,0,0,0.2);
...
.layer-modal__body { padding: 14px; display: grid; gap: 12px; }
.layer-modal__actions { display: flex; gap: 8px; justify-content: flex-end; }
...
(function () {
  ...
  function showModal(open) {
    ...
    if (open) $('#layer-status').textContent = '';
  }
  ...
  function addVectorLayerFromFeatures(map, features, nameHint) {
    ...
    const lyr = new ol.layer.Vector({ source: src, zIndex: 50 });
  ...
  async function handleAddLayer() {
    ...
    try {
      ...
      if (ext === 'kmz') {
        // Optional: if JSZip is present, we can try to unzip and read doc.kml
        if (!window.JSZip) {
        ...
        if (!entry) { status('No KML found inside the KMZ.', true); return; }
      ...
      status('Unsupported file type. Use KML or GeoJSON.', true);
    } catch (err) {
      console.error(err);
...
.layer-chip input{ accent-color:#1d4ed8; width:14px; height:14px; }
