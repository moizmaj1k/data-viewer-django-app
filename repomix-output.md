...
Generated by Repomix on: 2025-12-01 02:47:56
...
{% block toolbar %}
  <div class="toolbar-compact">
    ...
    <!-- Filters toggle: label + switch -->
    <div class="filters-toggle">
      <span class="filters-toggle-label">Filters</span>
      <button
        id="btn-filters"
        class="filters-toggle-switch"
        type="button"
        aria-expanded="false"
        title="Show/Hide Filters"
      >
        <span class="fts-track">
          <span class="fts-knob"></span>
        </span>
      </button>
    </div>
  </div>
...
<section id="filters-panel" class="filters-panel" aria-hidden="true">
  <div class="filters-inner">
    ...
    <div class="filters-separator" aria-hidden="true"></div>

    <!-- RIGHT: Map Tools (Add Layer / Add Marker now live here) -->
    <div class="filters-tools">
      <span class="filters-tools-label">Map Tools</span>

      <div class="filters-tools-row">
        <!-- Add Layer icon button (same IDs/classes as before) -->
        <button id="btn-add-layer" class="btn btn-outline" type="button"
                aria-haspopup="dialog" aria-controls="layer-modal" title="Add Layer to Map">
          ➕🗺️ Add Layer
        </button>

        <!-- Measurement tool: sits right beside Add Layer -->
        <div class="map-tool map-tool--measure">
          <button
            type="button"
            id="map-measure-toggle"
            class="btn btn-outline map-tool-btn"
          ...
          <div
            id="map-measure-panel"
            class="map-tool-dropdown map-measure-dropdown is-hidden"
          >
            <div class="map-measure-row">
              <label class="map-measure-label">Measurement type</label>
              <select id="map-measure-type" class="map-measure-select">
            ...
            <div class="map-measure-row">
              <label class="map-measure-checkbox">
                <input
                  type="checkbox"
                  id="map-measure-segments"
                  checked
                />
            ...
            <div class="map-measure-row">
              <label class="map-measure-checkbox">
                <input
                  type="checkbox"
                  id="map-measure-clear-previous"
                  checked
                />
                <span>Clear previous measure</span>
              </label>
            </div>

            <div class="map-measure-row map-measure-actions">
              <button
                type="button"
                id="map-measure-start"
                class="map-measure-btn map-measure-btn--primary"
              >
                Start
              </button>
            ...
            <div class="map-measure-row">
              <button
                ...
                class="map-measure-btn map-measure-btn--danger"
              >
                Clear all
              </button>
            </div>
      ...
      <div class="filters-tools-row">
        <!-- Add Marker icon button (same IDs/classes as before) -->
        <button id="btn-add-marker" class="btn btn-outline" type="button"
                aria-haspopup="dialog" aria-controls="marker-modal" title="Add Marker to Map">
          ➕📍 Add Marker
        </button>
      </div>
  ...
  <!-- Map container -->
  <div id="map">
    <!-- Map Layers icon button (top-right overlay) -->
    <button
      id="map-layers-toggle"
      class="map-layers-toggle"
      type="button"
      title="Map layers & markers"
      aria-expanded="false"
      aria-controls="map-layers-panel"
    >
      <img src="{% static 'layers.png' %}" alt="Layers" class="map-layers-icon">
    </button>

    <!-- Map Layers / Markers panel -->
    <div id="map-layers-panel" class="map-layers-panel" aria-hidden="true">
      <div class="mlp-head">
        <span class="mlp-title">Map Items</span>
        <button type="button" id="map-layers-close" class="mlp-close" aria-label="Close">×</button>
      </div>

      <div class="mlp-section">
        <div class="mlp-section-title">Layers</div>
        <!-- moved here: JS will still populate this by ID -->
        <div id="layer-toggles" class="layer-toggles"></div>
      </div>

      <div class="mlp-section">
        <div class="mlp-section-title">Markers</div>
        <!-- moved here: JS will still populate this by ID -->
        <div id="marker-toggles" class="marker-toggles"></div>
      </div>
    </div>
  </div>
  ...
  <!-- Add-Marker logic (similar to Add-Layer modal) -->
  <script defer src="{% static 'markers.js' %}"></script>
  <script defer src="{% static 'measure-tools.js' %}"></script>
...
/* Make Add Layer and Add Marker look identical and compact */
#btn-add-layer,
#btn-add-marker,
 #map-tool-btn {
  padding: 6px 10px;
...
.filters-actions .btn-solid,
.filters-actions .btn-warning {
  padding: 5px 10px !important;   /* reduced from 8px 14px */
...
.page {
  ...
  flex-direction: column;
  height: calc(100vh - var(--topbar-h));
  /* All outer spacing lives here now */
  padding: 0 4px;
  box-sizing: border-box;
}

/* Filters toggle (label + switch) in the top toolbar */
.filters-toggle {
  display: flex;
  flex-direction: column;
...
/* Track of the switch */
.filters-toggle-switch .fts-track {
  width: 38px;
  height: 18px;
  border-radius: 999px;
  background: #e5e7eb;
  ...
  display: flex;
  align-items: center;
  transition: background var(--fx-fast) var(--fx-ease);
}

/* Knob of the switch */
.filters-toggle-switch .fts-knob {
  width: 14px;
  height: 14px;
  border-radius: 999px;
  background: #9ca3af;
...
/* ON state when filters are open: aria-expanded = true */
.filters-toggle-switch[aria-expanded="true"] .fts-track {
  background: #1d4ed8;
}

.filters-toggle-switch[aria-expanded="true"] .fts-knob {
  transform: translateX(18px);
  background: #ffffff;
}

/* Layers icon in map overlay button */
.map-layers-icon {
  display: block;
  width: 18px;
  height: 18px;
...
/* Small layers icon button over the map (top-right) */
.map-layers-toggle {
  position: absolute;
  top: 10px;
  right: 10px;
...
/* Floating panel for layers & markers */
.map-layers-panel {
  position: absolute;
...
/* Popup for user markers (lat/lon + copy) */
.marker-popup {
  position: absolute;
  background: #ffffff;
  border-radius: 6px;
...
/* Shared styles for layer / marker lists in the Map Items panel */
.map-layers-panel .layer-toggles,
.map-layers-panel .marker-toggles {
  max-height: 90px;
  overflow-y: auto;
  border-radius: 4px;
  border: 1px solid #e5e7eb;
...
/* Row wrappers */
.map-layers-panel .marker-row,
.map-layers-panel .layer-row {
  display: flex;
  align-items: center;
  justify-content: space-between;
  gap: 6px;
...
/* Group action buttons tight on the right (edit / center / delete) */
.marker-row-actions,
.layer-row-actions {
  display: inline-flex;
  align-items: center;
...
/* Clickable label (toggle + name) */
.map-layers-panel .marker-toggle,
...
.map-layers-panel .marker-toggle:hover,
.map-layers-panel .layer-toggle:hover {
  background: #f9fafb;
}

/* Hide the native checkbox; we use the pill as the visible control */
.map-layers-panel .marker-toggle input[type="checkbox"],
.map-layers-panel .layer-toggle input[type="checkbox"] {
  position: absolute;
  opacity: 0;
  pointer-events: none;
}

/* Tiny rectangular toggle pill */
.toggle-pill {
  width: 24px;
  height: 14px;
  border-radius: 3px;
...
.toggle-pill.is-on::after {
  transform: translateX(10px);
  background: #ffffff;
}

/* Hooks if we ever want per-type styling later */
.marker-toggle-pill {}
.layer-toggle-pill {}

/* Icon + name text */
.map-layers-panel .marker-name,
.map-layers-panel .layer-name {
  display: inline-flex;
  align-items: center;
  gap: 4px;
  overflow: hidden;
...
/* Inject emoji icons before the text, just like before */
.map-layers-panel .marker-name::before {
  content: "📍";
}

.map-layers-panel .layer-name::before {
  content: "🗺️";
}

.marker-row.is-editing {
  background: #fef2f2; /* soft reddish highlight */
  border-radius: 4px;
}

/* Pill shows "editing" state when row is editing */
.marker-row.is-editing .marker-toggle-pill {
  background: #fee2e2;
  border-color: #ef4444;
  width: auto;
  min-width: 60px;
...
.marker-row.is-editing .marker-toggle-pill::after {
  display: none; /* hide the little knob */
}

.marker-row.is-editing .marker-toggle-pill::before {
  content: "editing";
  position: absolute;
  inset: 0;
  display: flex;
...
/* Icon buttons: edit / delete / center – larger 24x24 size */
.marker-edit-btn,
.marker-delete-btn,
.layer-delete-btn,
.marker-center-btn,
...
/* Edit icon button uses static/edit-marker.png */
.marker-edit-btn {
  background-image: url("/static/edit-marker.png");
}

/* Delete icon buttons for rows */
.marker-delete-btn,
.layer-delete-btn {
  background-image: url("/static/delete-icon.png"); /* update path if needed */
}

/* Center icon (place your PNG at this path or update it) */
.marker-center-btn,
.layer-center-btn {
  background-image: url("/static/center-icon.png");
}

.marker-edit-btn:hover,
.marker-delete-btn:hover,
.layer-delete-btn:hover,
.marker-center-btn:hover,
.layer-center-btn:hover {
  filter: brightness(0.9);
}

/* (Optional) if you ever add a locate button again, keep it compact */
.mlp-locate-btn {
  border: 0;
  background: #e5e7eb;
  border-radius: 4px;
...
/* Apply / Reset area – compact and wraps nicely on smaller screens */
.filters-actions {
  display: flex;
  gap: 8px;
  align-items: center;
  ...
  /* NEW: force it to be on its own row under all fields */
  grid-column: 1 / -1;
  justify-content: flex-start;
}

/* "Stick current selection" checkbox next to Reset */
.filters-actions .stick-selection {
  display: inline-flex;
  align-items: center;
  gap: 4px;
  font-size: 11px;
...
.filters-actions .stick-selection input {
  margin: 0;
}
...
/* Middle vertical separator – visible “physical” divider */
.filters-separator {
  flex: 0 0 auto;
  width: 1px;
  background: #e5e7eb;
...
/* Right side: Map Tools – takes the other 50% */
.filters-tools {
  flex: 0 0 50%;
  max-width: 50%;
  display: flex;
  flex-direction: column;
  gap: 4px;               /* keep tools compact so height doesn’t grow */
}
...
/* Compact Map Tools buttons (Add Layer & Add Marker) */
#btn-add-layer,
#btn-add-marker {
...
/* Filters panel layout: left (data filters) + right (Map Tools) */
.filters-inner {
  display: flex;
  align-items: stretch;   /* both sides same height */
  gap: 8px;
  padding: 4px 8px;       /* slightly smaller padding to save height */
}


/* Left side: existing filters – exactly 50% width */
.filters-form {
  flex: 0 0 50%;
  max-width: 50%;
  display: grid;
  grid-template-columns: repeat(auto-fit, minmax(200px, 1fr));
  gap: 6px;               /* small gap to keep box height low */
  align-items: end;
  padding: 0;
}
...
/* --- COMPACT Multi-Select (Roads & Assets) --- */
.multi-select .ms-trigger {
  padding: 4px 8px !important;
  font-size: 11px !important;
  height: 28px !important;      /* same compact height as Apply/Reset */
}

.multi-select .ms-trigger span {
  font-size: 11px !important;
}

/* dropdown panel smaller padding */
...
.multi-select .ms-list .check {
  padding: 4px 4px !important;
}

.multi-select .ms-list .check span {
  font-size: 11px !important;
}

/* Select All / Clear All buttons */
.multi-select .ms-actions button {
  padding: 4px 8px !important;
  font-size: 11px !important;
  height: 26px !important;
}

/* Compact radio buttons for Assets */
#assets-select .radio span {
  font-size: 11px !important;
  line-height: 1.1 !important;
}

/* Make District dropdown as compact as Roads / Assets triggers */
.filters-form #filter-district {
  font-size: 11px;
  padding: 5px 9px;      /* tighter padding */
  height: 28px;          /* matches compact buttons */
  line-height: 1.2;
}
...
#map {
  ...
  border-radius: 6px;          /* “slightly curved” */
  border: 1px solid #c2bccc;       /* black border */
...
/* Fullscreen control: leave space on the right for the reset-north/rotate icon */
#map .ol-full-screen {
  top: 8px;
  right: 40px;   /* shifted left; rightmost slot stays free for recenter */
}

/* Make sure the map fills the screen nicely in fullscreen mode */
...
/* --- Overview Map (mini-map) bottom-right above Google credits --- */
#map .ol-overviewmap.ol-custom-overviewmap,
#map .ol-overviewmap.ol-custom-overviewmap.ol-uncollapsible {
  position: absolute;
  bottom: 24px !important;  /* slightly above Google/OL attributions */
  right: 4px !important;
  left: auto !important;
  top: auto !important;
  z-index: 20;
...
#map .ol-overviewmap.ol-custom-overviewmap .ol-overviewmap-map {
  width: 220px;
  height: 160px;
  border-radius: 4px;
}

#map .ol-overviewmap.ol-custom-overviewmap:not(.ol-collapsed) {
  border: 1px solid #d1d5db;
  background: #ffffff;
}

#map .ol-overviewmap.ol-custom-overviewmap .ol-overviewmap-box {
  border: 2px solid #ff0000;  /* golden-yellow box for current view */
}

/* Base button reset */
#map .ol-overviewmap.ol-custom-overviewmap button {
  ...
  /* 🔽 kill default text + any built-in sprite */
...
/* OPEN → show ">" icon */
#map .ol-overviewmap.ol-custom-overviewmap:not(.ol-collapsed) button {
  background-image: url("/static/greater-than-icon.png") !important;
}

/* COLLAPSED → show "<" icon */
#map .ol-overviewmap.ol-custom-overviewmap.ol-collapsed button {
  background-image: url("/static/less-than-icon.png") !important;
}



/* --- Mouse Position: show EPSG:4326 coords near the scale/zoom length --- */
/* --- Mouse Position: show EPSG:4326 coords near the scale/zoom length --- */
#map .ol-mouse-position {
  position: absolute;
  bottom: 32px !important;   /* same vertical zone as scale line */
  left: 5px !important;   /* nudged right so it sits beside the scale */
  top: auto !important;
...
.drawer {
  ...
  border-radius: 6px;      /* slightly curved edges */
...
/* Override generic .kv styles for the drawer's view mode */
.drawer-edit .kv-view {
  border-collapse: collapse;
  font-size: 13px;          /* keep typography consistent */
}

/* Remove the top borders between rows inside drawer view */
.drawer-edit .kv-view tr + tr td,
.drawer-edit .kv-view tr + tr th {
  border-top: none;
}

/* Full-width, stacked layout for headers + values */
.drawer-edit .kv-view th {
  width: auto;              /* don't lock to 38% */
  padding: 0 0 2px 0;       /* tighter spacing above value */
}

.drawer-edit .kv-view td {
  padding: 0 0 6px 0;       /* small space under value */
}
...
.drawer-section h4{ margin: 0; font-size: 16px; color: #111; }

 /* Collapsible section header (title + right-aligned arrow) */
 .cs-head{
   display:flex;
   align-items:center;
   justify-content:space-between;
 ...
 .cs-title{
   margin:0;
   line-height:1.2;
   flex:1 1 auto;     /* keep arrow on the same line, right-aligned */
   min-width:0;       /* prevent wrapping that pushes arrow below */
 }
 .cs-toggle{
   border:0;
   background:transparent;
  ...
  .meta-cell{
    ...
    font-size:12px;
    flex:1 1 auto;
    min-width:160px;             /* fill row, then wrap to next line */
  }
  .meta-cell b{ margin-right:6px; }
...
/* Reuse global .btn/.btn-outline look; just tweak sizing */
.map-tool-btn {
  display: inline-flex;
  align-items: center;
  justify-content: center;
  padding: 5px 10px;    /* similar to Add Layer / Marker */
  font-size: 12px;
  gap: 4px;
  line-height: 1;
}
...
/* Dropdown: compact + small font */
.map-tool-dropdown {
  position: absolute;
  top: calc(100% + 4px);   /* just below the Measurement button */
  left: 0;
  z-index: 50;             /* above filters panel content */
  padding: 6px;
  border-radius: 6px;
  border: 1px solid #d0d4db;
  background: #ffffff;
  font-size: 11px;
  margin-top: 0;           /* ensure it doesn’t push layout */
}

.map-tool-dropdown.is-hidden {
  display: none;
}
...
/* Buttons inside dropdown: compact, similar feel to other btns */
.map-measure-btn {
  flex: 1;
  padding: 3px 6px;
  font-size: 11px;
...
.map-measure-btn[disabled] {
  opacity: 0.5;
  cursor: not-allowed;
}

.map-measure-btn--primary {
  background: #2563eb;
  color: #ffffff;
  border-color: #1d4ed8;
}

.map-measure-btn--danger {
  width: 100%;
  background: #fee2e2;
  color: #991b1b;
  border-color: #fecaca;
...
<head>
  <meta charset="utf-8">
  <title>🗃️ DCA Dashboard</title>
  <meta name="viewport" content="width=device-width, initial-scale=1" />
  <link rel="stylesheet" href="/static/main.css">
  {% block head_extra %}{% endblock %}
</head>
<body>
  <!-- Browser reload (no template tags needed) -->
  <script
    src="{% static 'django-browser-reload/reload-listener.js' %}"
    data-workers-url="/__reload__/reload-worker.js"
    defer>
  </script>
  <header class="topbar">
    <div class="brand">🗃️ Data Collection App - Dashboard</div>
    <nav class="toolbar">
