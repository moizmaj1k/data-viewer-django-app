...
Generated by Repomix on: 2025-11-18 23:10:21
...
/* Shared styles for layer / marker lists */
.layer-toggles,
.marker-toggles {
  max-height: 90px;
  overflow-y: auto;
...
/* Each row: LAYERS → simple: [checkbox][🗺️ name] */
.layer-toggles label {
  display: flex;
  align-items: center;
  justify-content: flex-start;   /* checkbox + name sit close together */
  gap: 6px;
  padding: 3px 4px;
  border-radius: 4px;
  cursor: pointer;
...
/* Each row: MARKERS → can still have checkbox + locate etc */
...
.marker-toggles label:hover {
  background: #f9fafb;
}

/* Icon + name on the left */
.layer-toggles label span,
.marker-toggles label span {
  display: inline-flex;
  align-items: center;
  gap: 4px;
  overflow: hidden;
...
/* Inject emoji icons */
.layer-toggles label span::before {
  content: "🗺️";
}

.marker-toggles label span::before {
  content: "📍";
}

/* Visibility checkbox styled as a tiny "view" icon */
